...
(function (d, cfg) {
    ...
    const sessionId =
        ...
        (localStorage.setItem("session_id", uuid()), localStorage.getItem("session_id"));

    window.Chat360_SessionId = sessionId;

    /* ── 2. cfg merge ─────────────────────────────────────────── */
    cfg = Object.assign(
        { url: "https://connect.dental360grp.com/webchat", color: "#146ef5", title: "AI Assistant", },
    ...
    function init() {
      ...
      cursor:pointer;box-shadow:0 4px 12px rgba(0,0,0,.15);transition:transform .25s ease`;
        ...
        frame.style.cssText = `position:fixed;bottom:${gap + 60}px;right:${gap}px;width:${W}px;
  max-width:calc(100vw - 40px);height:${H}px;
  ...
  box-shadow:0 4px 12px rgba(0,0,0,.25);transition:transform .25s ease,opacity .25s ease`;
        ...
        let open = false;
        const robotIcon = '<i class="fa-solid fa-robot" aria-label="Open chat"></i>';
        const closeIcon = '<i class="fa-solid fa-xmark" aria-label="Close chat"></i>';
